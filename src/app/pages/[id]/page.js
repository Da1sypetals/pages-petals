...
export default function PageViewer() {
  ...
  return (
    <div className="fixed inset-0 flex flex-col">
      ...
      {html ? <iframe srcDoc={html} className="flex-1 w-full border-0" title={title} sandbox="allow-scripts allow-same-origin" /> : <div className="flex-1 flex items-center justify-center bg-white"><span className="w-5 h-5 border-2 border-zinc-200 border-t-zinc-400 rounded-full animate-spin" /></div>}
    </div>
  );
}
