...
import { getPage, getContent, pageTitle } from "@/lib/kv";
...
export async function GET(_req, { params }) {
  ...
  const [meta, raw] = await Promise.all([getPage(id), getContent(id)]);
  if (!meta || raw === null) return NextResponse.json({ error: "not found" }, { status: 404 });

  const title = pageTitle(meta.filename);
  const html = meta.filename.endsWith(".md")
    ? "<!DOCTYPE html><html><head><meta charset=utf-8><meta name=viewport content='width=device-width,initial-scale=1'><title>" + title + "</title>" + STYLE + "</head><body>" + marked.parse(raw, { breaks: true, gfm: true }) + "</body></html>"
    : raw;
