          ...
          <Link href={`/pages/${page.id}`} className="block p-5 pb-14">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[13px] text-zinc-500 mt-2.5 leading-relaxed line-clamp-2">{snippet}</p>
              </div>
            </div>
          </Link>
          <div className="absolute bottom-3 right-3 flex items-center gap-1">
            <Link href={`/pages/${page.id}`} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-full text-[12px] text-zinc-500 hover:text-zinc-900 hover:bg-zinc-100 transition-all duration-200">
              <span>Open</span>
              <ArrowUpRight weight="bold" className="w-3 h-3" />
            </Link>
            <button
              onClick={() => setConfirmOpen(true)}
              className="w-7 h-7 rounded-full flex items-center justify-center text-zinc-400 hover:text-rose-500 hover:bg-rose-50 transition-all duration-200"
            >
