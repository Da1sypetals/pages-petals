      ...
      <motion.div layout initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.96 }} transition={{ type: "spring", stiffness: 500, damping: 35 }}>
        <div className="group relative rounded-2xl bg-white border border-zinc-200/80 hover:border-zinc-300 hover:shadow-sm transition-all duration-300">
          ...
          <div className="absolute bottom-3 right-3 flex items-center gap-1">
            <Link href={`/pages/${page.id}`} className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-full text-[12px] text-zinc-500 hover:text-zinc-900 hover:bg-zinc-100 transition-all duration-200"><span>Open</span><ArrowUpRight className="w-3 h-3" /></Link>
            {onDelete && <button onClick={() => setConfirmOpen(true)} className="w-7 h-7 rounded-full flex items-center justify-center text-zinc-400 hover:text-rose-500 hover:bg-rose-50 transition-all duration-200"><Trash className="w-3.5 h-3.5" /></button>}
          </div>
        </div>
      </motion.div>
    </>
